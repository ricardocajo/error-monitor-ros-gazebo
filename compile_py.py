...
def compile_py(node: Node, ctx=None, file_prefix=None, filepath=None, from_command=False, property_='', line=None, object_=None):
    ...
    if ctx is None:
        ...
        if len(node.args) == 3:
            ...
            var = ''
            event = []
            arg_1 = compile_py(node.args[2], ctx, property_=node.args[0], line=node.args[1])
            if node.args[0] == 'eventually':
                eventually_check = True
                var = ctx.add_eventually()
                event = [{'var': var, 'comp': arg_1[0]}]
            comparisons = arg_1[0], node.args[0]+' '+arg_1[1],arg_1[2],arg_1[3],arg_1[4] or eventually_check,[event+arg_1[5][0], arg_1[5][1], arg_1[5][2]]
        if len(node.args) == 4:
            arg_1 = compile_py(node.args[2], ctx, property_=node.args[0], line=node.args[1])
            ...
            comparisons = arg_1[0]+' and ('+arg_2[0]+')', node.args[0]+' '+arg_1[1]+', '+arg_2[1],arg_1[2]+arg_2[2],arg_1[3]+arg_2[3],arg_1[4] or arg_2[4],[arg_2[5][0], u_e_and, arg_2[5][2]]
        if len(node.args) == 5:
        ...
        if len(node.args) == 1:
            ...
            if op == 'or':
                if event_bool_l and not event_bool_r:
                    ...
                    for err in err1_r:
                        err1_output += '\\n    '+err+': {'+str(counter)+'}'
                    event_or = [{'var': var, 'comp': expr_r, 'aux': index, 'err': '\'{0}Error at line '+str(line)+':{1}\\n'+state_r+'\\n{2}Failing state:{3}'+err1_output+"\\n    \'.format(Fore.BLUE,Style.RESET_ALL,Fore.RED,Style.RESET_ALL,"+'\'\''+')'}] #','.join(err2_r).replace('states','save_states')
                if event_bool_r and not event_bool_l:
                    ...
                    for err in err1_l:
                        err1_output += '\\n    '+err+': {'+str(counter)+'}'
                    event_or = [{'var': var, 'comp': expr_l, 'aux': index, 'err': '\'{0}Error at line '+str(line)+':{1}\\n'+state_l+'\\n{2}Failing state:{3}'+err1_output+"\\n    \'.format(Fore.BLUE,Style.RESET_ALL,Fore.RED,Style.RESET_ALL,"+'\'\''+')'}] #','.join(err2_l).replace('states','save_states')
            if op == 'and':
                if event_bool_l:
                    event_and = ' and '+expr_r
                if event_bool_r:
                    event_and = ' and '+expr_l
            return prec + str(expr_l) +' '+op+' ' + str(expr_r), str(state_l)+' '+op_state+' '+str(state_r),err1_l+err1_r,err2_l+err2_r,event_bool_l or event_bool_r,[event_l[0]+event_r[0],event_l[1]+event_r[1]+event_and,event_l[2]+event_r[2]+event_or]
        ...
        if len(node.args) == 4:
            if op == '==' or op == '!=':
                ...
                if op == '!=':
                    op = '>='
                return prop_prefix(property_) + str(expr_r)+'-'+str(node.args[3]) + op + str(expr_l) + op + str(expr_r)+'+'+str(node.args[3]),str(state_l)+' '+op_state+'{{'+str(node.args[3])+'}}'+' '+str(state_r),err1_l+err1_r,err2_l+err2_r,event_bool_l or event_bool_r,[event_l[0]+event_r[0],event_l[1]+event_r[1],event_l[2]+event_r[2]]
            signal = '+'
            if property_ == 'always':
                if op == '>' or op == '>=':
                    signal = '-'
                return prop_prefix(property_) + str(expr_l) + op + str(expr_r) + signal + str(node.args[3]),str(state_l)+' '+op_state+'{{'+str(node.args[3])+'}}'+' '+str(state_r),err1_l+err1_r,err2_l+err2_r,event_bool_l or event_bool_r,[event_l[0]+event_r[0],event_l[1]+event_r[1],event_l[2]+event_r[2]]
            if property_ == 'never':
                if op == '>' or op == '>=':
                    signal = '-'
                return prop_prefix(property_) + str(expr_l)+ signal + str(node.args[3]) + op + str(expr_r),str(state_l)+' '+op_state+'{{'+str(node.args[3])+'}}'+' '+str(state_r),err1_l+err1_r,err2_l+err2_r,event_bool_l or event_bool_r,[event_l[0]+event_r[0],event_l[1]+event_r[1],event_l[2]+event_r[2]]
        return prop_prefix(property_) + str(expr_l) + op + str(expr_r), str(state_l)+' '+op_state+' '+str(state_r),err1_l+err1_r,err2_l+err2_r,event_bool_l or event_bool_r,[event_l[0]+event_r[0],event_l[1]+event_r[1],event_l[2]+event_r[2]]
        ...
        expr_l,state_l,err1_l,err2_l,event_bool_l,event_l = compile_py(node.args[1], ctx)
        expr_r,state_r,err1_r,err2_r,event_bool_r,event_r = compile_py(node.args[2], ctx)
        return str(expr_l) + op + str(expr_r), str(state_l)+' '+op+' '+str(state_r),err1_l+err1_r,err2_l+err2_r,event_bool_l or event_bool_r,[event_l[0]+event_r[0],event_l[1]+event_r[1],event_l[2]+event_r[2]]
